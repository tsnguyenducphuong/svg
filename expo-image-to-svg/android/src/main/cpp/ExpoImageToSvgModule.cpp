...
//  Used for pass1, pass2, pass3 (and pass4/pass5) inside
   ...
   size_t count)
{
   ...
   // BUG-FIX-1: gradientDetectThresh was read twice (lines 250 and 291 in the original).
   // The second read was dead code — removed.
...
// =============================================================================
//  JSI host function: nativeVectorizeMultiPass  (ENH-12 + ENH-13 pipeline)
//
...
//    lcqColorsPerTile       number  → mpOpts.lcqColorsPerTile       (default 32)
//
...
//    stitchThresh           number  → mpOpts.stitchThresh           (default 3.5)
//    seamRepairThresh       number  → mpOpts.seamRepairThresh       (default 2.5)
//
//  ENH-16  Adaptive Tile Quantization:
//    varFlat                number  → mpOpts.varFlat                (default 20.0)
//    varMid                 number  → mpOpts.varMid                 (default 150.0)
//
...
//    pass4   PassOptions  → highlights layer tuning
//    pass5   PassOptions  → low-lights layer tuning
// =============================================================================
   ...
   size_t count)
{
   ...
   // Controls the tile partition and per-tile palette depth.
   // BUG-FIX-2: The engine's vectorizeMultiPass uses these values through the
   // MultiPassOptions struct (options.lcqGridW etc.) when routing to
   // buildLCQPaletteAndAssign. The JS comment previously showed default 24
   // for lcqColorsPerTile, but FIX-COLOR-1 raised it to 32 — corrected below.
   mpOpts.lcqGridW =
   ...
   // ── ENH-13: Hue-Aware Cross-Tile Palette Stitching ───────────────────────
   // Pass A: merge adjacent-tile centroids within stitchThresh ΔE into a
   // shared anchor before pixel assignment, eliminating tile-boundary seams.
   // Pass B: reclassify border pixels within seamRepairThresh ΔE of their
   // cross-boundary neighbour after pixel assignment.
   // BUG-FIX-3: These two fields were not exposed at all in the original
   // module. The engine now reads them from MultiPassOptions; JS callers can
   // override them or leave them at their compile-time defaults (3.5 / 2.5).
   mpOpts.stitchThresh =
   ...
   // ── ENH-16: Adaptive Tile Quantization variance thresholds ────────────────
   // varFlat  — tiles with luminance variance below this get 3 colours and a
   //            coarse speckle filter (flat regions: sky, backgrounds).
   // varMid   — tiles between varFlat and varMid get 6 colours (midtones).
   // Tiles above varMid get the full 8-colour detail treatment.
   // BUG-FIX-4: These fields were not exposed at all in the original module.
   // The engine reads options.varFlat / options.varMid in vectorizeMultiPass
   // and passes them through to buildLCQPaletteAndAssign → varianceToOptions.
   mpOpts.varFlat =
   ...
   // Pass 4 — Highlights (screen-blend shimmer; engine applies optimal values
   // on top of any per-pass overrides supplied here).
   ...
   // Pass 5 — Low-Lights / Shadows (multiply-blend; same caveat as pass4).
   ...
   jlong jsiRuntimePtr)
{
   ...
   // ── nativeVectorizeMultiPass (ENH-12 + ENH-13 6-pass Stochastic Painterly) ─
